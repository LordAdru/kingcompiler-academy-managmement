...
export const StudentManager: React.FC<StudentManagerProps> = ({
  ...
  const handleDelete = async (id: string) => {
    if (
      confirm(
        "Are you sure? This will delete the student and all their schedules.",
      )
    ) {
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredStudents.map((student) => {
                const coll = collaborators.find(
                  (c) => c.uid === student.collaboratorId,
                );
                return (
                  <StudentTableRow
                    key={student.id}
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Age: {student.age} • Joined {student.joiningDate}
          </p>
        </div>
      </div>
    </td>
      ...
      <div className="flex items-center gap-3">
        <span
          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
            student.billing.feeStatus === "paid"
  ...
  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm space-y-6">
    <div className="flex items-center justify-between">
      ...
      <div className="flex gap-2">
        <button onClick={() => onEdit(student)} className="p-2 text-slate-400">
          <Edit2 size={18} />
        </button>
        <button
          onClick={() => onDelete(student.id)}
        ...
        >
          ...
          <div className="flex-1 p-6 sm:p-8 space-y-8 overflow-y-auto">
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    className="w-full px-5 py-3 rounded-2xl bg-slate-50 border border-slate-100 outline-none focus:ring-2 focus:ring-amber-500 transition-all"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    type="tel"
                    className="w-full px-5 py-3 rounded-2xl bg-slate-50 border border-slate-100 focus:ring-2 focus:ring-amber-500 transition-all"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  className="w-full px-5 py-3 rounded-2xl bg-slate-50 border border-slate-100 font-bold"
            ...
            <div className="space-y-4">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                <CreditCard size={14} className="text-amber-500" /> Financial
                Configuration
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateBilling("feeAmount", Number(e.target.value))
                    }
                  />
                </div>
                <div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center px-1">
                ...
                <button
                  type="button"
                  onClick={addSlot}
                  className="text-[10px] font-black text-amber-600 bg-amber-50 px-4 py-2 rounded-xl border border-amber-100"
                >
                  Add Slot
                </button>
              </div>
              <div className="space-y-4">
                {isSchedulesLoading ? (
                  <div className="py-10 text-center bg-slate-50 rounded-3xl animate-pulse">
                    Loading Slots...
                  </div>
                ) : (
                  initialSchedules.map((slot, idx) => (
                    <div
                      key={slot.id}
                      className="p-5 rounded-[2rem] bg-slate-900 text-white space-y-4"
                    >
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <input
                          ...
                          onChange={(e) =>
                            updateTime(slot.id, "startTime", e.target.value)
                          }
                        />
                        <input
                          type="time"
                          ...
                          value={slot.endTime}
                          onChange={(e) =>
                            updateTime(slot.id, "endTime", e.target.value)
                          }
                        />
                      </div>
                    </div>
          ...
          <div className="p-6 sm:p-8 bg-slate-50 border-t border-slate-100 flex justify-end gap-3 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-6 py-4 font-bold text-slate-400 uppercase text-[10px]"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black uppercase text-[10px]"
            >
              Save Record
            </button>
          </div>
