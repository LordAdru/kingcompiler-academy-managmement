...
export const dbService = {
  ...
  getStudents: async (): Promise<Student[]> => {
    try {
      ...
      const students = snapshot.docs.map(
        (doc: any) => ({ id: doc.id, ...doc.data() }) as Student,
      );
  ...
  getGroups: async (): Promise<GroupBatch[]> => {
    try {
      ...
      return snapshot.docs
        .map((doc: any) => ({ id: doc.id, ...doc.data() }) as GroupBatch)
        .sort((a: GroupBatch, b: GroupBatch) =>
  ...
  saveGroup: async (group: GroupBatch) => {
    await setDoc(doc(db, COLLECTIONS.GROUPS, group.id), sanitize(group), {
      merge: true,
    });
  },
  ...
  getSchedules: async (
    ...
    try {
      ...
      return snapshot.docs.map(
        (doc: any) => ({ id: doc.id, ...doc.data() }) as ClassSchedule,
      );
    } catch (err) {
      return [];
    }
  ...
  saveSchedule: async (schedule: ClassSchedule) => {
    await setDoc(
      doc(db, COLLECTIONS.SCHEDULES, schedule.id),
      sanitize(schedule),
      { merge: true },
    );
  ...
  getSessions: async (): Promise<ClassSession[]> => {
    try {
      ...
      const snapshot = await getDocs(q);
      return snapshot.docs
        .map((doc: any) => ({ id: doc.id, ...doc.data() }) as ClassSession)
  ...
  updateSession: async (updatedSession: ClassSession) => {
    await updateDoc(
      doc(db, COLLECTIONS.SESSIONS, updatedSession.id),
      sanitize(updatedSession),
    );
  },
  ...
  getAttendance: async (studentId?: string): Promise<AttendanceRecord[]> => {
    try {
      ...
      const snapshot = await getDocs(q);
      return snapshot.docs.map(
        (doc: any) => ({ id: doc.id, ...doc.data() }) as AttendanceRecord,
      );
    } catch (err) {
      return [];
    }
  ...
  saveCollaborator: async (partner: AppUser) => {
    await setDoc(doc(db, COLLECTIONS.USERS, partner.uid), sanitize(partner), {
      merge: true,
    });
  },
  ...
  generateSessionsForSchedule: async (schedule: ClassSchedule) => {
    ...
    while (isBefore(current, projectionLimit)) {
      if (schedule.days.includes(current.getDay())) {
        ...
        await setDoc(
          doc(db, COLLECTIONS.SESSIONS, deterministicId),
          sanitize({
            id: deterministicId,
